...
const HomePage = memo(() => {
  ...
  const onItemCreated = useCallback(async (item: TodoItem) => {
    return await actions.items.save(item.listId, item);
  }, []);
  const onItemCompleted = useCallback((item: TodoItem) => {
    item.state = TodoItemState.Done;
    item.completedDate = new Date();
    actions.items.save(item.listId, item);
  }, []);
  const onItemSelected = useCallback((item?: TodoItem) => {
    actions.items.select(item);
  }, []);
  const onItemDeleted = useCallback((item: TodoItem) => {
    if (item.id) {
  ...
  return <Stack>
            <Stack.Item>
                <Stack horizontal styles={titleStackStyles} tokens={stackPadding}>
                    <Stack.Item grow={1}>
                        <Shimmer width={300} isDataLoaded={!!appContext.state.selectedList} shimmerElements={[{
            ...
            height: 20
                    ...
                    <Stack.Item>
                        <IconButton disabled={!isReady} menuProps={menuProps} iconProps={iconProps} styles={{
            root: {
              fontSize: 16
            }
          }} title="List Actions" ariaLabel="List Actions" />
                    </Stack.Item>
                </Stack>
            </Stack.Item>
            <Stack.Item tokens={stackItemPadding}>
                <TodoItemListPane list={appContext.state.selectedList} items={appContext.state.selectedList?.items} selectedItem={appContext.state.selectedItem} disabled={!isReady} onSelect={onItemSelected} onCreated={onItemCreated} onComplete={onItemCompleted} onDelete={onItemDeleted} />
            </Stack.Item>
        </Stack>;
});
